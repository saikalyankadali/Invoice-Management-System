  ...
  if (xmlhttp.readyState==4 && xmlhttp.status==200)
    {
		...
		//else if(type=='hsn') document.getElementById("prodid").value=n[0];
	...
	//else if(type=='hsn'){xmlhttp.open("GET","getprice.php?phsn="+value,true);}
	xmlhttp.send();
}
...
var xmlhttp;
  ...
  document.getElementById("transtable").on
  	if(value.length==0 & quantity.length==0)
		xmlhttp.open("GET","transtable.php",true);
	else 
		xmlhttp.open("GET","transtable.php?pid="+value+"&q="+quantity,true);	
	xmlhttp.send();
