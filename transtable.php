...
if(isset($_GET['pid']) & isset($_GET['q'])){
	$pid = $_GET['pid'];
	$quan = $_GET['q'];
	$plist = mysql_query("select product_name, cost_price, discount from product where product_id='{$pid}'");
	if(!$plist) die("error");
	if(mysql_num_rows($plist)){	
		while($row = mysql_fetch_array($plist)){
			...
			$dis = $GET['dis'];
			//$price *=$quan;	
			$price = ($price - ($price * ($dis/100))) * $quan; 
		}
	}
	mysql_query("insert into transaction values('{$pname}', $pid,$quan,$dis,$price,NULL)");
}
